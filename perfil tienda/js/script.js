...
let url2 = 'http://localhost:8080/productos/obtenerProductoPorIdTienda/1';
fetch(url2)
  .then(response => {
    if (!response.ok) {
      ...
      async function mostrarProductosHtml(productos) {
        try {
          ...
          <div data-id="${producto.id}" class="paquetesDisponibles__card">
            ...
            <a href="#" class="eliminar__btn"><button data-id="${producto.id}" type="button" class="eliminar__btnA">Eliminar</button></a>
          </div>`;
